    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data.names.length}</div>
            <p className="text-xs text-muted-foreground">
              {data.names.length > 0 ? `Primeros: ${data.names.slice(0, 3).join(", ")}` : "No se encontraron nombres"}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data.dates.length}</div>
            <p className="text-xs text-muted-foreground">
              {data.dates.length > 0 ? `Primeras: ${data.dates.slice(0, 3).join(", ")}` : "No se encontraron fechas"}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data.times.length}</div>
            <p className="text-xs text-muted-foreground">
              {data.times.length > 0 ? `Primeras: ${data.times.slice(0, 3).join(", ")}` : "No se encontraron horas"}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className={`grid w-full ${data.aiAnalysis ? 'grid-cols-3' : 'grid-cols-2'}`}>
                ...
                <Table>
                  ...
                  <TableBody>
                    <TableRow>
                      ...
                      <TableCell>{data.names.length}</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="font-medium">
                        <div className="flex items-center gap-2">
                      ...
                      <TableCell>{data.dates.length}</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="font-medium">
                        <div className="flex items-center gap-2">
                      ...
                      <TableCell>{data.times.length}</TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
              </ScrollArea>
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Mostrando primeros {Math.min(data.rawData.length, maxRows)} filas de {data.rawData.length} totales
              </p>
            </CardHeader>
            <CardContent>
              <ScrollArea className="max-h-96 overflow-y-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      {data.rawData[0]?.map((_, index) => (
                        <TableHead key={index}>Columna {index + 1}</TableHead>
                      ))}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {data.rawData.slice(0, maxRows).map((row, rowIndex) => (
              ...
              {data.rawData.length > maxRows && (
                <p className="text-sm text-muted-foreground mt-2 text-center">
                  ... y {data.rawData.length - maxRows} filas más
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <TabsContent value="ai" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  Análisis realizado con {data.aiAnalysis.provider}
                </p>
              </CardHeader>
